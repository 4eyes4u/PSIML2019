...
def find_single_barycenter(pts):
    ...
    r_mean = pts.T[0].mean()
    c_mean = pts.T[1].mean()
    return np.asarray([r_mean, c_mean])


def find_barycenters(pixels):
    barycenters = np.asarray([find_single_barycenter(pts) for pts in pixels])
    return barycenters
...
def process_src_image():
    ...
    for i, pts in enumerate(pixels):
        ...
        ch_pts = np.asarray([pts[i] for i in ch_idx])
...
def find_affine(pts):
    ...
    a, c = pts[1][0] - tx, pts[1][1] - ty
    b, d = pts[2][0] - tx, pts[2][1] - ty

    return np.asarray([[a, b, tx],
                       [c, d, ty],
                       [0, 0, 1]])
...
def extreme_triangles(pts):
    ...
    '''
    ch = ConvexHull(pts)
    ch_idx = np.asarray(ch.vertices, dtype=np.int32)
    ch_pts = np.asarray([pts[i] for i in ch_idx])
...
def find_size_mapping(size_src, size_dst):
    ...
    size_mapping = np.zeros((n_shapes, ), dtype=np.int32)
...
if __name__ == '__main__':
    ...
    triangle_src = np.asarray([triangle_src[0][1],
                               triangle_src[0][2],
                               triangle_src[0][3]])
    triangle_dst = extreme_triangles(barycenters_dst)
    triangle_dst = np.asarray([[tr[1], tr[2], tr[3]] for tr in triangle_dst])
